...
namespace InteractPayroll
{
    public partial class CustomMessageBox : Form
    {
        ...
        {
            using (CustomMessageBox form = new CustomMessageBox(message, headerText, messageBoxButtons, messageBoxIcon))
            {
                return form.ShowDialog();
            }
        }

        public CustomMessageBox(string message, string headerText, MessageBoxButtons messageBoxButtons, MessageBoxIcon messageBoxIcon)
        {
            InitializeComponent();
            ...
            //if (message.IndexOf("There are Time Sheets with Errors on the Client Database") > -1
            //|| message.IndexOf("There are Timesheet/Break Records that are NOT Included") > -1
            //|| message.IndexOf("Employee/s with 2 or More Records Exist.") > -1
            //|| message.IndexOf("must be Greater than Or Equal To\n\nPrevious Run Date") > -1
            //|| message.IndexOf("You may continue without uploading but these Timesheets will then be Lost.") > -1
            //|| message.IndexOf("Are you sure you want to Reset the Password for") > -1
            ...
            {
                this.picPicture.Image = InteractPayroll.Properties.Resources.Exclamation96;
            }
            else
            {
                if (messageBoxIcon == MessageBoxIcon.Error)
                {
                    this.picPicture.Image = InteractPayroll.Properties.Resources.Error96;
                }
                else
                {
                    if (messageBoxIcon == MessageBoxIcon.Information)
                    {
                        this.picPicture.Image = InteractPayroll.Properties.Resources.Info96;
                    }
                    else
                    {
                        ...
                        else
                        {
                            ...
                            {
                                this.picPicture.Image = InteractPayroll.Properties.Resources.Question96;
                            }
                            else
                            {
