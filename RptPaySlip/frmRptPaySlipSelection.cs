                ...
                if (this.reportViewer.LocalReport.DataSources.Count > 0)
                {
                    if (this.pvtTempDataSet.Tables["EmployeePayslip"] != null)
                    {
                        if (this.pvtTempDataSet.Tables["EmployeePayslip"].Rows.Count > 0)
                        {
                            ...
                            int intX = (rect.Width - frmEmployeePayslip.Width) / 2;
                            int intY = this.Top + (this.Height - frmEmployeePayslip.Height + 32) / 2;

                            frmEmployeePayslip.StartPosition = FormStartPosition.Manual;
                            frmEmployeePayslip.Location = new Point(intX, intY);
                            ...
                            if (AppDomain.CurrentDomain.GetData("EmailPayslips").ToString() == "Y")
                            {
                                ...
                                int intReturnCode = (int)clsISUtilities.DynamicFunction("Put_Email_Payslips_On_Queue", objParm);

                                if (intReturnCode == 0)
                                {
                                    CustomMessageBox.Show("Email of Payslips placed on Queue Successfully.", "Email Payslips", MessageBoxButtons.OK, MessageBoxIcon.Information);
                                }
                            }
                        }
                        else
                        {
                            CustomMessageBox.Show("None of these Employees have been Setup to be Emailed Payslips.", "Email Payslips", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        }
                    }
                }
            }
        ...
        private void btnOK_Click(object sender, System.EventArgs e)
        {
            try
            {
                ...
                if (pvtblnLoadedEmailButton == false)
                {
                    ...
                    emailToolStripMenuItem.TextAlign = ContentAlignment.MiddleCenter;
                    
                    if (this.rbnNoEmail.Checked == true)
                    {
                ...
                else
                {
                    ...
                    for (int intCount = 0; intCount < toolStrip.Items.Count; intCount++)
                    {
                        ...
                        if (tsiToolStripItem.ToolTipText == "Email Payslips")
                        {
                            if (this.rbnNoEmail.Checked == true)
                            {
                                toolStrip.Items[intCount].Enabled = false;
                            }
                            else
                            {
                                toolStrip.Items[intCount].Enabled = true;
                            }
